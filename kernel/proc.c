...
found:
  ...
  ///new add
  p->kernelpgtbl = kvminit_newpgtbl();// 为新进程创建独立的内核页表，并将内核所需要的各种映射添加到新页表上
  // 分配一个物理页，作为新进程的内核栈使用
  char *pa = kalloc();
  if(pa == 0)
    panic("kalloc");
...
freeproc(struct proc *p)
{
  ...
  //new add
  void *kstack_pa = (void*)kvmpa(p->kernelpgtbl, p->kstack); // 释放进程的内核栈
  kfree(kstack_pa);
  p->kstack = 0;
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    uvmdealloc(p->pagetable, sz, sz + n); // new add
